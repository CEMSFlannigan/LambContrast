...
AFFb4 = 1.6461;
AFFb5 = 8.4617;
DW = 0.6000; % turn this into a function of T later
...
LambAmp = 0;%0.005;%3;%6;%4e2;%3.75e1;%3.75; % nm
...
gvec_list{1} = (h2g_matr*g_basis_1')';
gvec_list{2} = (h2g_matr*g_basis_2')';
gvec_list{3} = -(h2g_matr*g_basis_1')';
gvec_list{4} = -(h2g_matr*g_basis_2')';
...
[gvec_list,hvec_list,orig_spot_num] = RecipSpotExpander(gvec_list,hvec_list, g2h_matr, h2g_matr, g_lim);

spot_angles = zeros(1,length(gvec_list));
ext_dist_arr = zeros(1,length(gvec_list));
SF_arr = zeros(1,length(gvec_list));
Vg_mag_list = zeros(1,length(gvec_list));
true_simspots = length(hvec_list);
...
thickness_arr = zeros(length(xvals),length(timevals));
...
%% Matrices of Scattering

D = bsxfun(@minus, gvec_list(:,1), gvec_list(:,1)');                                  % Subtract To Create Matrix
gx_dif_matr = D;

D = bsxfun(@minus, gvec_list(:,2), gvec_list(:,2)');                                  % Subtract To Create Matrix
gy_dif_matr = D;

D = bsxfun(@minus, gvec_list(:,3), gvec_list(:,3)');                                  % Subtract To Create Matrix
gz_dif_matr = D;

D = bsxfun(@minus, hvec_list(:,1), hvec_list(:,1)');                                  % Subtract To Create Matrix
hx_dif_matr = D;

D = bsxfun(@minus, hvec_list(:,2), hvec_list(:,2)');                                  % Subtract To Create Matrix
hy_dif_matr = D;

D = bsxfun(@minus, hvec_list(:,3), hvec_list(:,3)');                                  % Subtract To Create Matrix
hz_dif_matr = D;
...
for hvec_count = 1:length(hvec_list)
    ...
    %s_g_vec(hvec_count,:) = dir_vec*s_g(curg_vec);
    
    s_g_mag(hvec_count,:) = s_g(curg_vec)*1e9;
    
end
exc_err_matr = 2*pi*diag(s_g_mag);

objective_aperture_radius = 5.865; % 1/nm

gx_arr = zeros(13,1);
gy_arr = zeros(13,1);
gz_arr = zeros(13,1);
sum_arr = zeros(13,1);
integral_spots = zeros(orig_spot_num,1);
opts1=  optimset('display','off');

tic;
for k = 1:length(timevals)
    for i = 1:length(xvals)%parfor i = 1:length(xvals)
        
        summed_intensities = zeros(1,true_simspots);
        summed_intensities(1) = % NEED TO ACCOUNT FOR GAUSSIAN;
        sumemd_intensities(2) = % NEED TO ACCOUNT FOR GAUSSIAN
        summed_intensities(3) = % NEED TO ACCOUNT FOR GAUSSIAN;
        sumemd_intensities(4) = % NEED TO ACCOUNT FOR GAUSSIAN
        summed_intensities(5) = % NEED TO ACCOUNT FOR GAUSSIAN;
        sumemd_intensities(6) = % NEED TO ACCOUNT FOR GAUSSIAN
        summed_intensities(7) = % NEED TO ACCOUNT FOR GAUSSIAN;
        sumemd_intensities(8) = % NEED TO ACCOUNT FOR GAUSSIAN
        sumemd_intensities(9) = % NEED TO ACCOUNT FOR GAUSSIAN
        ...
        while abs(cury) <= abs(end_y(curx))
            ...
            s_g_vec = zeros(true_simspots,3);
            s_g_mag = zeros(true_simspots,1);
...
%             end
            ...
            tic;
            pot_phase_fact_r = real(pot_phase);
            pot_phase_fact_i = imag(pot_phase);
            ...
            tic;
%             disp('1');
%             exp_matr = gpuArray(exp_matr);
%             whos exp_matr
%             disp('2');
            exp_matr = expm(exp_matr);
%             whos exp_matr
%             disp('3');
%             exp_matr = gather(exp_matr);
%             whos exp_matr
%             disp('4');
...
%             assignin('base','exp_matr_old',exp_matr);
            ...
            summed_intensities = exp_matr*summed_intensities';
...
%             summed_intensities = summed_intensities+change_summed_intensities_exc_err' + change_summed_intensities_interact';
            summed_intensities = summed_intensities./sqrt(sum(summed_intensities.^2));
            %abs(summed_intensities(1))
            toc
            tic;
            %% Location to add Gaussian fit for spot
            numFit = 13;
            curNum = 0;
            curSpotNum = 0;
            FitParams = zeros(orig_spot_num,7);
            for p = 1:length(summed_intensities)
                curNum = curNum + 1;
                gx_arr(curNum) = gvec_list(p,1);
                gy_arr(curNum) = gvec_list(p,2);
                gz_arr(curNum) = gvec_list(p,3);
                sum_arr(curNum) = abs(summed_intensities(p));
                sep = 0.08;
                if curNum > 13
                    curNum = 0;
                    curSpotNum = curSpotNum + 1;
                    Gauss3D = @(x) x(7)*exp(-(((gx_arr-x(1)).^2./(2.*x(2).^2)) + ((gy_arr-x(3)).^2./(2.*x(4).^2)) + ((gz_arr-x(5)).^2./(2.*x(6).^2)))) - sum_arr;
                    FitParam = lsqnonlin(Gauss3D,[gx_arr(1),sep,gy_arr(1),sep,gz_arr(1),sep,1], [gx_arr(1)-sep,0,gy_arr(1)-sep,0,gz_arr(1)-sep,0,0], [gx_arr(1)+sep,Inf,gy_arr(1)+sep,Inf,gz_arr(1)+sep,Inf,Inf],opts1);
                    FitParams(curSpotNum,:) = FitParam;
                    Gauss3DFit = @(x,y,z) FitParam(7)*exp(-(((x-FitParam(1)).^2./(2.*FitParam(2).^2)) + ((y-FitParam(3)).^2./(2.*FitParam(4).^2)) + ((z-FitParam(5)).^2./(2.*FitParam(6).^2))));
                    integral_spots(curSpotNum) = integral3(Gauss3DFit,-g_lim,g_lim,-g_lim,g_lim,-g_lim,g_lim);
                end
            end
            
            norm_integral_spots = integral_spots./sqrt(sum(integral_spots.^2));
            factors = zeros(size(norm_integral_spots));
            factors(integral_spots > 0) = norm_integral_spots(integral_spots > 0)./integral_spots(integral_spots > 0);
            NormFitParams = FitParams;
            NormFitParams(:,7) = NormFitParams(:,7).*factors;
            
            curNum = 0;
            curSpotNum = 1;
            FitParam = FitParams(curSpotNum,:);
            adj_summed_intensities = zeros(size(summed_intensities));
            for p = 1:length(summed_intensities)
                curNum = curNum + 1;
                Gauss3DFit = @(x,y,z) FitParam(7)*exp(-(((x-FitParam(1)).^2./(2.*FitParam(2).^2)) + ((y-FitParam(3)).^2./(2.*FitParam(4).^2)) + ((z-FitParam(5)).^2./(2.*FitParam(6).^2))));
                adj_summed_intensities(p) = Gauss3DFit(gvec_list(p,1),gvec_list(p,2),gvec_list(p,3));
                if curNum > 13
                    curNum = 0;
                    curSpotNum = curSpotNum + 1;
                    FitParam = FitParams(curSpotNum,:);
            ...
            adj_summed_intensities = adj_summed_intensities./sqrt(sum(adj_summed_intensities.^2));
            summed_intensities = adj_summed_intensities;
            toc
            curx = curx + x_incr;
            cury = cury + y_incr;
        ...
        FitParamBF = FitParams(1,:);
        Gauss3DFit = @(x,y,z) FitParam(7)*exp(-(((x-FitParam(1)).^2./(2.*FitParam(2).^2)) + ((y-FitParam(3)).^2./(2.*FitParam(4).^2)) + ((z-FitParam(5)).^2./(2.*FitParam(6).^2))));
        
        bf_contrast(i,k) = integral3(Gauss3DFit,-objective_aperture_radius,objective_aperture_radius,-objective_aperture_radius,objective_aperture_radius,-objective_aperture_radius,objective_aperture_radius);
        thickness_arr(i,k) = tot_thick;
    end
    k
end
toc