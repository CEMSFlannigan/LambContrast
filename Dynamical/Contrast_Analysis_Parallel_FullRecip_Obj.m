...
V_c = latpar^3;
ExtDist = @(thet,h,k,l) pi*(V_c./1e9./1e9./1e9)*cosd(thet)/lambda/(Ge_ASF(thet)/1e10)/real(Ge_SF(h,k,l,r1,r2,r3,r4,r5,r6,r7,r8));
...
[gvec_list,hvec_list] = RecipRasterGenerator(28,5,g2h_matr,0.01, k_actual);%RecipSpotGenerator(4,gvec_list,simspots,hvec_list);

% setting up the Bragg spots
simspots = length(gvec_list);
...
% MAJOR ISSUE: NOT ENOUGH MEMORY FOR EVEN 500 x 500 PX DIFFRACTION
% RECREATIONS
    ...
    parfor i = 1:length(xvals)
        ...
        while abs(cury) <= abs(end_y(curx))
            ...
            for hvec_count = 1:simspots
                ...
                mod_gvec_list(hvec_count+1,:) = curg_vec;%[curg_vec(1)/(1+strain_x_mag), curg_vec(2)/(1+strain_y_mag), curg_vec(3)];%curg_vec+g_distort*curg_vec./curg_mag;
                
                mod_hvec_list(hvec_count+1,:) = curh_vec;%[curh_vec(1)/(1+strain_true_x_mag), curh_vec(2)/(1+strain_true_y_mag), curh_vec(3)/(1+strain_true_z_mag)];
...
%             summed_intensities = summed_intensities+change_summed_intensities_exc_err' + change_summed_intensities_interact';
        ...
        for p = 1:length(gvec_list)
            curgvec = gvec_list(p,:);
            if (sqrt(sum(curgvec.^2)) <= objective_aperture_radius)
                bf_contrast(i,k) = bf_contrast(i,k) + abs(summed_intensities(p));
            end
        end
    end
    k
