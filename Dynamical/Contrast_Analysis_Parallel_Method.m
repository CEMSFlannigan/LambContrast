function [bf_contrast,strain_matr,thickness_vector] = Contrast_Analysis_Parallel_Method(input_array)
...
gam_tilt = 0;
beta_tilt = 1.55;
beta_add = input_array{1}; 
alpha_add = input_array{2}; 
k_unit = [0, -1, 0];
...
timevals = 0; % ps  %  RESTART BIG 37:177!
...
LambAmp = 0;%0.005;%0.05%0.5%3;%6;%12%4e2;%3.75e1;%3.75; % nm % RESTART BIG 3
k = 0.027286040502180; %0.004969305039777; %0.012561985042817 %0.027286040502180; % 1/nm
omega = (35.739999668189228e9/1e12)*2*pi; % 36.906711070799567 % 36.389025944521329 % 35.739999668189228 % 1/ps
VL = 5.350; % nm/ps
VT = 3.570; % nm/ps
V = 8; % 32 % 16 % 8 % nm/ps
h = 36; % nm
...
thickness_vector = zeros(length(xvals),length(timevals));
...
for k = 1:length(timevals)
    tic
    parfor i = 1:length(xvals)%parfor i = 1:length(xvals)
...
%             summed_intensities = summed_intensities+change_summed_intensities_exc_err' + change_summed_intensities_interact';
        ...
        thickness_vector(i,k) = tot_thick;
    end
    k
    toc;
end
